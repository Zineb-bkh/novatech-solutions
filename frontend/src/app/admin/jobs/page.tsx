...
function JobPreview({ form }: { form: any }) {
        ...
        <div className="flex flex-wrap gap-3 mb-3">
          {form.jobType && (
            <span className="flex items-center gap-1 text-[11px]" style={{ color: 'var(--fg-muted)' }}>
              ...
              {CONTRACT_LABELS[form.jobType] || form.jobType}
            </span>
          )}
          {form.jobLocation && (
            <span className="flex items-center gap-1 text-[11px]" style={{ color: 'var(--fg-muted)' }}>
...
function JobEditor({ initial, isNew, onSave, onClose }: {
  initial: Partial<JobArticle>; isNew: boolean;
  ...
  const handleSave = async () => {
    ...
    if (!form.jobPosition) { toast.error("Le poste est obligatoire pour une offre d'emploi"); return; }
    setSaving(true);
    try {
      const payload: any = {
        title:             form.title,
        ...
        jobSalaryMin:      form.jobSalaryMin ? parseInt(form.jobSalaryMin) : null,
        jobSalaryMax:      form.jobSalaryMax ? parseInt(form.jobSalaryMax) : null,
        jobSalaryCurrency: form.jobSalaryCurrency,
      };
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-display font-bold text-lg" style={{ color: 'var(--fg)' }}>
              {isNew ? "Nouvelle offre d'emploi" : "Modifier l'offre"}
            </h2>
          </div>
          ...
          <div className={`w-full md:w-1/2 overflow-y-auto border-r border-[var(--border)] p-5 space-y-5 ${tab === 'preview' ? 'hidden md:block' : ''}`}>
            ...
            <Field label=" Tags" hint="Séparés par des virgules — ex : React, Node.js, Fullstack">
              <div className="relative">
                ...
                <input value={form.tagsRaw} onChange={e => set('tagsRaw', e.target.value)}
                  ...
                  placeholder="React, Node.js, Fullstack, Senior" />
              </div>
            </Field>
            ...
            <Field label=" Titre *">
              <textarea rows={2} value={form.title} onChange={e => set('title', e.target.value)}
                ...
                placeholder="Senior Fullstack Developer — React / Node.js" />
            </Field>
            ...
            <div className="rounded-xl border border-[var(--border)] p-4 space-y-3" style={{ background: 'var(--bg-2)' }}>
              ...
              <div className="flex gap-3 items-start">
                <div className="relative w-12 h-12 rounded-xl overflow-hidden shrink-0 border border-[var(--border)] cursor-pointer hover:opacity-80 transition-opacity"
                  ...
                  onClick={() => document.getElementById('avatar-upload-job')?.click()}
                  ...
                  <input id="avatar-upload-job" type="file" accept="image/*" className="hidden"
                    onChange={async e => {
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <input value={form.authorRole} onChange={e => set('authorRole', e.target.value)}
                    className={inputCls} style={inputStyle} placeholder="CTO" />
                </div>
              </div>
            </div>
            ...
            <Field label=" Résumé" hint="Affiché dans la liste du blog">
              <textarea rows={2} value={form.excerpt} onChange={e => set('excerpt', e.target.value)}
                ...
                placeholder="Nous cherchons un(e) développeur(se) Fullstack senior pour renforcer notre équipe..." />
            </Field>

            <Field label=" Contenu (Markdown) *"
              hint="## Titre  |  ### Sous-titre  |  **gras**  |  > citation  |  `code`">
...
export default function AdminJobsPage() {
  ...
  const [editing, setEditing] = useState<Partial<JobArticle> | null>(null);
  const [isNew, setIsNew] = useState(false);

  const loadJobs = async () => {
    setLoading(true);
    try {
      const data = await apiFetch('/api/admin/articles?type=job');
      const articles = data.articles ?? data;
      setJobs(articles.filter((a: JobArticle) => a.type === 'job'));
    } catch (err: any) {
      toast.error(err.message || 'Impossible de charger les offres');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = jobs.filter(j =>
    ...
    (
      ...
      (j.jobLocation ?? '').toLowerCase().includes(search.toLowerCase())
    )
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display font-bold text-2xl" style={{ color: 'var(--fg)' }}>Offres d'emploi</h1>
        ...
        <button
          onClick={() => {
            setEditing({
              title: '', excerpt: '', content: '', type: 'job',
              jobType: 'full-time', jobLocation: '',
              tags: [], published: true,
            });
            setIsNew(true);
          }}
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Rechercher une offre ou ville..."
          value={search}
      ...
      {loading ? (
        ...
        <div className="card text-center py-16">
          ...
          <p className="font-medium" style={{ color: 'var(--fg)' }}>Aucune offre trouvée</p>
          <p className="text-sm mt-1" style={{ color: 'var(--fg-muted)' }}>Créez votre première offre d'emploi</p>
        </div>
      ) : (
        <div className="grid gap-4">
          {filtered.map(job => (
            ...
            >
              {job.imageUrl && (
                <img src={job.imageUrl} alt="" className="w-14 h-14 rounded-xl object-cover flex-shrink-0" />
              )}
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-semibold text-sm" style={{ color: 'var(--fg)' }}>{job.title}</h3>
                    {job.jobPosition && (
                      <p className="text-xs text-brand-500 font-medium mt-0.5">{job.jobPosition}</p>
                    )}
                    <p className="text-xs mt-0.5 line-clamp-1" style={{ color: 'var(--fg-muted)' }}>{job.excerpt}</p>
                    {!job.published && (
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2">
                  ...
                  {job.jobType && (
                    <span className="flex items-center gap-1 text-xs" style={{ color: 'var(--fg-muted)' }}>
                      <Briefcase className="w-3 h-3" /> {CONTRACT_LABELS[job.jobType] ?? job.jobType}
                    </span>
                  )}
                  {job.jobSalaryMin && job.jobSalaryMax && (
                    <span className="flex items-center gap-1 text-xs" style={{ color: 'var(--fg-muted)' }}>
                      <Banknote className="w-3 h-3" />
                      {job.jobSalaryMin.toLocaleString()} – {job.jobSalaryMax.toLocaleString()} {job.jobSalaryCurrency ?? 'MAD'}
                    </span>
                  )}
                  <span className="text-xs" style={{ color: 'var(--fg-muted)' }}>
                    Publié le {job.publishedAt
                      ? format(new Date(job.publishedAt), 'd MMM yyyy', { locale: fr })
                      : format(new Date(job.createdAt), 'd MMM yyyy', { locale: fr })
                    }
                  </span>
                  <span className="text-xs" style={{ color: 'var(--fg-muted)' }}>
                    {(job.views ?? 0).toLocaleString()} vues
                  </span>
                </div>
              </div>
            </motion.div>
